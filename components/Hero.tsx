    ...
    <section className="relative pt-40 pb-32 lg:pt-48 lg:pb-40 overflow-hidden bg-slate-950">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-900/80 border border-slate-800/60 text-slate-300 text-xs md:text-sm font-medium mb-10 hover:border-slate-700 transition-colors cursor-pointer backdrop-blur-sm group">
          ...
          <span className="text-white">For service businesses:</span> stop losing leads and slow follow-up
          <ChevronRight className="w-3 h-3 text-slate-500 group-hover:text-white transition-colors" />
        </div>
        
        <h1 className="text-5xl md:text-7xl lg:text-8xl font-bold text-white tracking-tight mb-8 leading-[1.05]">
          We build and run the <br className="hidden md:block"/>
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-purple-400 to-emerald-400 animate-gradient-x">systems that turn leads into booked calls.</span>
        </h1>
        
        <p className="max-w-3xl mx-auto text-lg md:text-xl text-slate-400 mb-8 leading-relaxed font-light">
          SynapseHub helps service businesses fix missed calls, slow follow-up, and manual admin by managing lead response, appointment flow, and automation for them.
        </p>

        <div className="max-w-4xl mx-auto mb-12 flex flex-wrap justify-center gap-3 text-sm">
          {[
            'Recover missed leads faster',
            'Book more appointments automatically',
            'Reduce manual admin load',
            'Get a managed setup instead of patchwork tools'
          ].map((item) => (
            <div key={item} className="px-4 py-2 rounded-full border border-slate-800 bg-slate-900/60 text-slate-300">
              {item}
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-center items-center gap-6">
          ...
          >
            ...
            <div className="text-left flex flex-col justify-center">
                <span className="text-xs text-emerald-400 font-bold uppercase tracking-widest leading-none mb-2">Live Qualification</span>
                <span className="leading-none text-2xl group-hover:text-blue-200 transition-colors font-bold tracking-tight">Start Voice Strategy Call</span>
                <span className="text-slate-400 text-sm mt-1 group-hover:text-slate-300">Find the leak in your lead flow</span>
            </div>
          </button>

          <button 
            onClick={onOpenChat}
            className="px-8 py-4 rounded-xl bg-blue-600 text-white hover:bg-blue-500 transition-all font-medium flex items-center gap-2 group shadow-lg shadow-blue-900/30"
          >
            Get Qualified in Chat <ChevronRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
          </button>
